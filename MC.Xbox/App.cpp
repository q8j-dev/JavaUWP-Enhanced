...
public:
    ...
    void Render(const AuthUiState& state) {
        ...
        d2dContext_->CreateSolidColorBrush(D2D1::ColorF(0x79C05A, 0.14f), accentSoft.GetAddressOf());

        d2dContext_->BeginDraw();
        DrawDirtBackground();

        const float marginX = width_ * 0.04f;
        const float marginY = height_ * 0.055f;
        const D2D1_RECT_F frame = D2D1::RectF(marginX, marginY, width_ - marginX, height_ - marginY);
        // "Bandit Launcher" branding above the glass panel
        {
            ComPtr<ID2D1SolidColorBrush> logoBrush;
            d2dContext_->CreateSolidColorBrush(D2D1::ColorF(0x79C05A), logoBrush.GetAddressOf());
            if (logoBrush && titleFormat_)
                DrawText(L"Bandit Launcher", titleFormat_.Get(),
                    D2D1::RectF(frame.left + 8.0f, 6.0f, frame.right, marginY - 2.0f),
                    logoBrush.Get());
        }
...
private:
    ...
    void LoadMinecraftFont() {
        ...
        if (GetFileAttributesW(fontPath.c_str()) == INVALID_FILE_ATTRIBUTES) return;
        ComPtr<IDWriteFactory3> factory3;
        if (FAILED(dwriteFactory_.As(&factory3))) return;
        ComPtr<IDWriteFontSetBuilder> builder;
        if (FAILED(factory3->CreateFontSetBuilder(builder.GetAddressOf()))) return;
        ComPtr<IDWriteFontFile> fontFile;
        if (FAILED(factory3->CreateFontFileReference(fontPath.c_str(), nullptr, fontFile.GetAddressOf()))) return;
        BOOL isSupported = FALSE; DWRITE_FONT_FILE_TYPE fileType = DWRITE_FONT_FILE_TYPE_UNKNOWN; UINT32 faceCount = 0;
        if (FAILED(fontFile->Analyze(&isSupported, &fileType, nullptr, &faceCount)) || !isSupported || faceCount == 0) return;
        for (UINT32 i = 0; i < faceCount; ++i) {
            ComPtr<IDWriteFontFaceReference> faceRef;
            if (SUCCEEDED(factory3->CreateFontFaceReference(fontPath.c_str(), nullptr, i,
                DWRITE_FONT_SIMULATIONS_NONE, faceRef.GetAddressOf())) && faceRef)
                builder->AddFontFaceReference(faceRef.Get());
        }
        ComPtr<IDWriteFontSet> fontSet;
        if (FAILED(builder->CreateFontSet(fontSet.GetAddressOf())) || !fontSet) return;
        ComPtr<IDWriteFontCollection1> collection1;
        if (FAILED(factory3->CreateFontCollectionFromFontSet(fontSet.Get(), collection1.GetAddressOf())) || !collection1) return;
    ...
    void EnsureDirtBackground() {
        ...
        if (!LoadBitmapFromFile(path, dirtBackground_)) WriteLogF(L"Failed to load dirt background from %s", path.c_str());
        else WriteLog(L"Dirt background loaded");
    }
    ...
    void DrawDirtBackground() {
        ...
        if (!dirtBackground_) {
            ...
            d2dContext_->CreateSolidColorBrush(D2D1::ColorF(0x05080B), bg.GetAddressOf());
            if (bg) d2dContext_->FillRectangle(D2D1::RectF(0,0,width_,height_), bg.Get());
            return;
        }
        const D2D1_SIZE_F sz = dirtBackground_->GetSize();
        if (sz.width <= 0.0f || sz.height <= 0.0f) return;
        for (float y = 0.0f; y < height_; y += sz.height)
            for (float x = 0.0f; x < width_; x += sz.width) {
                float dw = (std::min)(sz.width, width_ - x), dh = (std::min)(sz.height, height_ - y);
                d2dContext_->DrawBitmap(dirtBackground_.Get(), D2D1::RectF(x,y,x+dw,y+dh),
                    1.0f, D2D1_INTERPOLATION_MODE_NEAREST_NEIGHBOR, D2D1::RectF(0,0,dw,dh));
            }
    }
        ...
        const wchar_t* mf = fc ? L"Minecraft Seven" : L"Segoe UI";
        const wchar_t* cf = fc ? L"Minecraft Seven" : L"Consolas";
        dwriteFactory_->CreateTextFormat(
            cf, fc, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL,
            DWRITE_FONT_STRETCH_NORMAL, 56.0f, L"en-US", codeFormat_.GetAddressOf());
        dwriteFactory_->CreateTextFormat(
            mf, fc, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL,
        ...
        dwriteFactory_->CreateTextFormat(
            mf, fc, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL,
            DWRITE_FONT_STRETCH_NORMAL, 46.0f, L"en-US", titleFormat_.GetAddressOf());
        dwriteFactory_->CreateTextFormat(
            mf, fc, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL,
            DWRITE_FONT_STRETCH_NORMAL, 22.0f, L"en-US", captionFormat_.GetAddressOf());
        dwriteFactory_->CreateTextFormat(
        ...
        dwriteFactory_->CreateTextFormat(
            mf, fc, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL,
            DWRITE_FONT_STRETCH_NORMAL, 36.0f, L"en-US", bodyMid_.GetAddressOf());
        dwriteFactory_->CreateTextFormat(
            mf, fc, DWRITE_FONT_WEIGHT_REGULAR, DWRITE_FONT_STYLE_NORMAL,
            DWRITE_FONT_STRETCH_NORMAL, 26.0f, L"en-US", smallMid_.GetAddressOf());
    ...
    void GlowSelect(D2D1_RECT_F r, float radius) {
        ...
        d2dContext_->CreateSolidColorBrush(D2D1::ColorF(0x79C05A, 0.10f), g1.GetAddressOf());
        d2dContext_->CreateSolidColorBrush(D2D1::ColorF(0x79C05A, 0.20f), g2.GetAddressOf());
    ...
    const LaunchAuthConfig& authConfig)
{
    ...
    std::vector<std::string> vmOptionStorage;
    vmOptionStorage.reserve(16);
    // heap and JIT already configured above
    vmOptionStorage.push_back("--enable-native-access=ALL-UNNAMED");
    vmOptionStorage.push_back("--add-opens=jdk.zipfs/jdk.nio.zipfs=ALL-UNNAMED");
    const std::wstring localJavaSecurityPatch = exeDir + L"\\java-base-security-realpath.jar";
    const std::wstring packagedJavaSecurityPatch = packageDir + L"\\java-base-security-realpath.jar";
